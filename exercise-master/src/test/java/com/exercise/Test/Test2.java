...
public String comment = "My comment is " + LocalDateTime.now();
	@Test
	public void testMessageSaving() throws InterruptedException, IOException {
		...
		try {
			...
			orderHistory.typeMessage(comment);
			orderHistory.sendMessage();
			orderHistory.confirmMsgInTable(comment);
			orderHistory.getFirstProductInfo();
			assertEquals(orderHistory.getSavedMessage(), comment);
			myAccount.logOut();
			driver.close();
		} catch (Exception e) {
